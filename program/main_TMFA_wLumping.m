...
[conc_boundaries_withLump,x0_withLump]=getBoundaries(fullModel,10^12,idx.toBelumped,optVal,percentage);
[conc_boundaries_withoutLump,x0_withoutLump]=getBoundaries(fullModel,10^12,[],optVal,percentage);
...
for type=["withLumping","withOutLumping"]
    ...
    if all(type=='withLumping')
        ...
        [Distr, optValTMFA, STAT, usedModel]=TMFA_LS(fullModel,10^12,idx.toBelumped,x0_withLump,optVal,percentage);
        rangesTable=myDeltaG_VA(usedModel,optValTMFA);
        savingName1=cell2mat({'../result/' str2mat(modelChoice) '_' str2mat(type) '_' 'Ranges.csv'});
        savingName2=cell2mat({'../result/' str2mat(modelChoice) '_' str2mat(type) '_' 'RxnsNames.csv'});
        ...
        [Distr, optValTMFA, STAT, usedModel]=TMFA_LS(fullModel,10^12,[],x0_withoutLump,optVal,percentage);
        savingName1=cell2mat({'../result/' str2mat(modelChoice) '_' str2mat(type) '_' 'Ranges.csv'});
        savingName2=cell2mat({'../result/' str2mat(modelChoice) '_' str2mat(type) '_' 'RxnsNames.csv'});
        writematrix(rangesTable,savingName1)
        writecell(fullModel.rxns,savingName2)
